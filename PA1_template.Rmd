...
```
The impact of this imputation is `r mean(totalStepsPerDayImputed$steps) - mean(totalStepsPerDay$steps)` for the mean. The median is slightly higher after the imputation and matches the value of the means for both datasets. Despite the negligible impact on mean and median, the total number of steps taken on days with lots of missing values is higher.
